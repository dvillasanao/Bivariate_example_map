...
package_date <-  Sys.time() # date of the CRAN snapshot that
# the checkpoint package uses
R_version <- R.Version()[["version.string"]] # R-Version to use
options(Ncpus = 6) # use 4 cores for parallelized installation of packages
```
...
Function `detach_all_packages`: This function is intended to unload (detach) all packages currently loaded in the R session, except for the basic system packages. This can be useful to clean up the working environment and avoid conflicts between packages or simply to free memory.  

The `csf()` function is intended to determine the directory of the file currently running in R, whether it is run from the command line, sourced in RStudio, run from the R console, or running a selection of code in RStudio. This function is useful for obtaining the path to the current script file regardless of how it is running.

```{r preparations, echo=FALSE, warning=FALSE, message=FALSE, results=FALSE}
# some constants
...
# this allows multiple persons to use the same RMarkdown without adjusting the working directory by themselves all the time
source(paste0(here::here(), "/analysis_grssnbchr/scripts/csf.R"))
...
source("manifest.R")
#unlink("manifest.R") #to delete the manifest.R file after the libraries have been loaded.
sessionInfo()
```
...
ggplot(# define main data source
 ...
 geom_raster(data = relief,
  ...
  geom_sf(mapping = aes(fill = mean_quantiles),
          color = "white",
          size = 0.1
          ) +
...
municipality_prod_geo %<>%
  mutate(gini_quantiles = cut(gini,
                            breaks = quantiles_gini,
                            include.lowest = TRUE),
         mean_quantiles = cut(mean,
                              breaks = quantiles_mean,
                              include.lowest = TRUE),
         # by pasting the factors together as numbers we match the groups defined
         # in the tibble bivariate_color_scale
         group = paste(as.numeric(gini_quantiles), "-", as.numeric(mean_quantiles))
         ) %>%
...
annotations <- tibble(label = c("Grey areas mean\nlow income and\nlow inequality",
                                "Blue areas mean\nhigh income and\nlow inequality",
                                "Violet areas mean\nhigh income and\nhigh inequality",
                                "Red areas mean\nlow income and\nhigh inequality"),
                      arrow_from = c("548921,232972", # grey
                                     "771356,238335", # blue
                                     "781136,125067", # violet
                                     "616348,81869"), # red
                      arrow_to = c("622435,206784", # grey
                                   "712671,261998", # blue
                                   "786229,149597", # violet
                                   "602334,122674"), # red
                      curvature = c(0.2, # grey
                                    0.1, # blue
                                    -0.1, # violet
                                    -0.2), # red
                      nudge = c("-3000,0", # grey
...
map <- ggplot(data = municipality_prod_geo # use the same dataset as before
             ) +
        # color municipalities according to their gini / income combination
        geom_sf(aes(fill = fill),
                color = "white", # use thin white stroke for municipalities
                size = 0.1) +
        # first: draw the relief
        geom_raster(data = relief,
                    aes(x = x,
...
```{r thematic-bivariate-map-with-legend, warning=FALSE, message=FALSE}
ggdraw() +
